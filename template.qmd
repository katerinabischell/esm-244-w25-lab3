...
The 'survived' column tells us if the passenger (1) survived or (0) died. The columns that will help predict which ones survived are 'pclass', 'sex', 'age', and 'cabin'. The big relationship I am seeing is that a lot of the passengers do not have cabins "NA" and most of the ones that survived (1) have cabin assignments - this makes me assume that the rich passengers have a higher survival rate since the ones that did not have assigned cabins are probably workers. Another correlation I am noticing is that the female passenders have had a higher survival rate. I am not sure that the 'ticket' column stands for. 

```{r}
sex_df <- t_df |> # this df shows us that 233 females survived and only 109 males survived. 
  select(survived, sex) |> 
  group_by(survived, sex) |> 
  summarise(count = n(), .groups = "drop") |>  # Count the number of males and females for each survived category
...
t_df |> 
  ...
  ggplot(aes(x = factor(survived), y = count, fill = sex)) +
...
```{r}
age_df <- t_df |> # this shows that the only category that survived more than died are children
...
age_df |> 
  ...
  geom_bar(stat = "identity", position = "stack") +
  labs(x = "Age Group", y = "Count", fill = "Survived",
       title = "Survival Counts by Age Group") +
  theme_minimal()
...
```{r}
sac_df <- t_df |> #this one is just looking at the following columns: survived, sex, age, and cabin
  select(survived, sex, age, cabin) |> 
  group_by(survived)
```
